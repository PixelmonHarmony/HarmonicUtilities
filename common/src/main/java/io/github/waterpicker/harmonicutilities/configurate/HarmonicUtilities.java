 ...
 * @since 1.0.0
 */
public class HarmonicUtilities {

    static final String MOD_ID = "harmonicutilities";
    ...
    static {
        try {
            HarmonicUtilities.listener = WatchServiceListener.create();
            Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                try {
                    HarmonicUtilities.listener.close();
    ...
    public static WatchServiceListener fileWatcher() {
        final WatchServiceListener ret = HarmonicUtilities.listener;
        if (ret == null) {
